...
class Vectorizer:
    ...
    def save_dataframe(self, data, name=""): 
        if os.path.isdir('./vectorization') == False: 
            os.mkdir("./vectorization")
            
        sparse.save_npz("./vectorization/vec_%s_nb_hash_%s_vectorizer_%s" % (name, str(self.nb_hash), str(self.vectorizer_type)),
                        data)

    def load_dataframe(self, name=""):
        return sparse.load_npz("./vectorization/vec_%s_nb_hash_%s_vectorizer_%s.npz" % (name, str(self.nb_hash), str(self.vectorizer_type)))
